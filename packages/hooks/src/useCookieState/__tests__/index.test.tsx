...
describe('useCookieState', () => {
  const setUp = (key: string, option: Option) =>
    renderHook(() => {
      const [state, setState] = useCookieState(key, option);
      return {
        state,
        setState,
      } as const;
  ...
  it('should support function updater', () => {
    ...
    act(() => {
      hook.result.current.setState((state) => `${state}, hh`);
    });
    expect(hook.result.current.state).toBe('hello world, hh');
  });
