...
describe('useEventTarget', () => {
  ...
  it('存在 transformer',async () => {
    const hook=renderHook(() => useEventTarget({transformer: (str: string) => str.toUpperCase()}));
    expect(hook.result.current[0]).toBeUndefined();
  ...
  it('可对输入进行类型转换', async () => {
    const hook = renderHook(() => useEventTarget<string,number>({transformer:(num:number) => String(num)}));
    expect(hook.result.current[0]).toBeUndefined();
